...
扩展端向接入的 AI 客户端宣告了以下 17 个 MCP 工具：
...
| `update_group` | 变更 | 重命名或更新现有自定义书签分组（系统内置常用/未分组受保护不可重命名）。 |
| `delete_group` | 变更 | 删除指定自定义书签分组，组内书签安全归入未分组（系统内置常用/未分组不可删除）。 |
| `rename_tag` | 变更 | 全局重命名或合并标签（若书签已有新标签则自动去重合并，操作前自动备份快照）。 |
| `delete_tag` | 变更 | 全局从所有书签中彻底剥离指定标签（不删除书签本身，操作前自动备份快照）。 |
