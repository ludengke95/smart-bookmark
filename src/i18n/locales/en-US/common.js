/**
 * English · Common & cross-scenario domain copy (common / groups / bookmark / latency)
 */
export default {
  common: {
    confirm: 'Confirm',
    cancel: 'Cancel',
    save: 'Save',
    delete: 'Delete',
    edit: 'Edit',
    close: 'Close',
    copy: 'Copy',
    copied: 'Copied',
    loading: 'Loading...',
    systemBuiltin: 'System Built-in',
    success: 'Operation succeeded',
    failed: 'Operation failed',
    clear: 'Clear',
    search: 'Search',
    all: 'All',
    refresh: 'Refresh',
    reset: 'Reset',
    export: 'Export',
    import: 'Import',
    preview: 'Preview',
    apply: 'Apply',
    collapse: 'Collapse',
    expand: 'Expand',
    warning: 'Notice'
  },
  groups: {
    pinned: 'Frequently Used',
    ungrouped: 'Ungrouped',
    addGroup: 'Add Group',
    groupNamePlaceholder: 'Enter group name...',
    deleteConfirm: 'Are you sure you want to delete this group? Bookmarks inside will be moved to Ungrouped.',
    builtinNoDelete: 'System built-in groups (Frequently Used / Ungrouped) cannot be deleted',
    editGroup: 'Edit Group',
    moveUp: 'Move Up',
    moveDown: 'Move Down',
    created: 'Group created',
    renamed: 'Group renamed',
    deleted: 'Group deleted',
    orderUpdated: 'Group order updated',
    moveUpTooltip: 'Move group up',
    moveDownTooltip: 'Move group down',
    renameTooltip: 'Rename',
    deleteTooltip: 'Delete group'
  },
  tags: {
    title: 'Tag Management',
    searchPlaceholder: 'Search tags...',
    noTags: 'No tags yet. Tags added to bookmarks will appear here.',
    noMatch: 'No matching tags found',
    bookmarkCount: '{count} bookmark(s)',
    renameTag: 'Rename Tag',
    renamePlaceholder: 'Enter new tag name...',
    mergeNotice: 'If the target tag exists, it will be automatically merged and deduplicated',
    deleteConfirm: 'Are you sure you want to remove tag "{tag}" from all bookmarks? Bookmarks themselves will not be deleted.',
    deleted: 'Tag "{tag}" removed',
    renamed: 'Tag updated to "{tag}"',
    merged: 'Tag "{oldTag}" merged into "{newTag}"',
    nameRequired: 'Tag name cannot be empty',
    sameName: 'New tag name is identical to the current name',
    renameTooltip: 'Rename or merge tag',
    deleteTooltip: 'Delete tag',
    expandAll: 'Expand all tags',
    collapseAll: 'Collapse tags'
  },
  bookmark: {
    addTitle: 'Add New Bookmark',
    editTitle: 'Edit Bookmark',
    name: 'Bookmark Name',
    nameRequired: 'Bookmark Name *',
    namePlaceholder: 'e.g. Internal GitLab',
    icon: 'Icon Identifier',
    changeIcon: 'Change Icon',
    selectBrandIcon: 'Select Brand Icon',
    useDefault: 'Use Default',
    tags: 'Tags (comma separated)',
    tagsPlaceholder: 'e.g. repo, CI/CD',
    endpoints: 'Routing Endpoints',
    endpointsTitle: 'Routing Endpoints (Intelligent Multi-URL Probing)',
    addEndpoint: '+ Add Endpoint',
    group: 'Group',
    openDefault: 'Open optimal endpoint',
    copyUrl: 'Copy URL',
    visitDirect: 'Visit',
    probeSpeed: 'Connectivity Latency',
    probeTest: 'Ping',
    probeTooltip: 'Test current latency for this endpoint',
    noBookmarks: 'No bookmarks yet. Click "Add Bookmark" on the top right to get started.',
    intranetBadge: 'Intranet',
    extranetBadge: 'Public',
    targetIp: 'Target IP / Domain',
    urlPlaceholder: 'e.g. http://192.168.10.50:8080',
    endpointName: 'Endpoint Name',
    allEndpoints: 'All Endpoints ({count})',
    optimal: 'Optimal',
    noEndpointsConfigured: 'No endpoints configured',
    directAddress: 'Direct Address',
    toastSaved: 'Bookmark updated',
    toastAdded: 'Bookmark added',
    toastDeleted: 'Bookmark deleted',
    deleteConfirm: 'Are you sure you want to delete bookmark "{name}"?',
    noValidEndpointToast: 'This bookmark has no valid endpoint configured',
    directJumpToast: '⚡ Routing to: {target}',
    directEndpointToast: 'Direct jump: {url}',
    nameRequiredToast: 'Please enter a bookmark name',
    noValidUrlToast: 'Please configure at least one valid URL',
    addToGroup: 'Add bookmark',
    groupEmpty: 'No bookmarks in this group',
    noMatch: 'No matching bookmarks',
    clearFilter: 'Clear filters'
  },
  latency: {
    direct: 'Direct',
    fast: 'Fast',
    good: 'Good',
    slow: 'Slow',
    sluggish: 'Lagging',
    timeout: 'Timeout',
    unreachable: 'Unreachable',
    offline: 'Offline'
  },
  select: {
    placeholder: 'Please select...'
  },
  themes: {
    // keys mirror THEMES.id (kebab-case) for t('themes.' + theme.id) lookups
    'paper-sand': 'Paper Sand',
    'ceramic-light': 'Ceramic Light',
    'obsidian-dark': 'Obsidian Dark'
  },
  errors: {
    cloudNoProvider: 'Cloud sync provider not configured',
    cloudAuthFailed: 'Authentication failed. Please check credentials or token',
    cloudNetworkError: 'Cannot connect to cloud server. Check network or URL',
    cloudDirNotFound: 'Target directory not found. Please create it first on server',
    cloudFileNotFound: 'Remote backup file not found',
    cloudUploadFailed: 'Cloud upload failed',
    cloudFetchFailed: 'Failed to fetch cloud backup',
    cloudTokenRequired: 'GitHub Personal Access Token is required',
    cloudGistIdRequired: 'Gist ID is required',
    cloudConflict: 'Remote backup is newer than local version',
    e2eeLocked: 'E2EE master password not unlocked',
    e2eeDecryptFailed: 'Decryption failed: incorrect master password',
    cloudImportFailed: 'Failed to parse and import cloud backup',
    apiHttpError: 'API returned an error (HTTP {status})',
    apiEmptyResponse: 'Model returned no valid content',
    aiEmptyInput: 'Please paste the LLM reply or upload a valid JSON file first',
    aiParseInvalid: 'Could not parse a valid change list from the LLM output',
    aiNoMatch: 'No current bookmark could be matched (bookmark IDs may mismatch)',
    aiAnalysisFailed: 'AI analysis was interrupted, please try again later',
    snapshotNotFound: 'Target snapshot data not found',
    builtinGroupNoDelete: 'System built-in groups cannot be deleted',
    mcpBookmarkNotFound: 'Bookmark with ID "{id}" not found',
    mcpUnknownTool: 'Unknown tool name: {name}'
  },
  subscriptions: {
    forkBookmark: 'Fork to My Bookmarks',
    forkSuccess: 'Cloned to personal bookmarks',
    readOnlyBadge: 'Team',
    readOnlyTooltip: 'Team subscribed bookmark (read-only)',
    readOnlyGroupTooltip: 'Team subscribed group, managed by subscription feed',
    dragDisabled: 'Subscribed collections do not support manual reordering'
  }
};
