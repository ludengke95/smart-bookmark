...
<!-- 容器包裹：固定头部与单一内部滚动视口 -->
<div class="h-full flex flex-col min-h-0 space-y-2.5">
  <!-- 搜索栏与概览 (固定不滚动) -->
  ...
  <!-- 提示说明 (固定不滚动) -->
  <div class="text-[11px] text-text-tertiary leading-relaxed px-1 flex-shrink-0">
    {t('tags.mergeNotice')}
  </div>

  <!-- 标签列表 (独占剩余高度，仅内部滚动) -->
  {#if appState.allTags.length === 0}
    <div class="flex-1 flex items-center justify-center p-8 text-center border border-dashed border-border-subtle rounded-xl text-text-tertiary text-xs">
      {t('tags.noTags')}
    </div>
          ...
          {#if editingTag === tag.name}
            <div class="flex items-center gap-1.5 flex-1 mr-2">
              ...
              <button
                ...
                class="px-2 py-1 rounded border border-border-subtle text-text-secondary text-[11px]"
              >
                {t('common.cancel')}
              </button>
            </div>
